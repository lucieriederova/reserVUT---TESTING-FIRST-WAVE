...
export const MIN_AHEAD_MS_CEO = 24 * 60 * 60 * 1000;
export const MIN_AHEAD_MS_GUIDE_SESSION = 2 * 24 * 60 * 60 * 1000;
...
export function validateEmailDomain(_email: string): boolean {
...
export function validateReservation(data: {
  ...
  // GLOBAL_EVENT: HEAD_ADMIN only, informational — skip most constraints
  if (data.type === 'GLOBAL_EVENT') {
    if (data.role !== 'HEAD_ADMIN') {
      return { code: 'FORBIDDEN', message: 'Only HEAD_ADMIN can create global events.' };
    }
    if (start <= now) {
...
export function createReservation(data: {
  ...
  // GLOBAL_EVENT: informational only, does not block rooms
  if (data.type === 'GLOBAL_EVENT') {
    const reservation: MemReservation = {
      id: uuidv4(),
      roomName: data.roomName || 'Event',
...
export function cancelReservation(id: string, requestingUserId: string, requestingRole: Role): MemReservation {
  ...
  const requestingPriority = PRIORITY_MAP[requestingRole];
  const isOwner = r.userId === requestingUserId;
  const canOverride = requestingPriority > r.priorityLevel;

  if (!isOwner && !canOverride) {
    throw Object.assign(new Error('Forbidden: insufficient priority to cancel this reservation.'), { code: 'FORBIDDEN' });
  }
  r.status = 'CANCELLED';
